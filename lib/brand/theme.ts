...
export const THEME_RULES: ThemeRule[] = [
  ...
  { test: p => p.startsWith('/owners'),           theme: { name:'owners',        accent:'#E0B12C', hoverBg:'rgba(224,177,44,.14)', photo:'minerals/photos/citrine-crystal.svg' } },
...
export function themeForPath(pathname: string): MineralTheme {
  // Strip /mxtk prefix if present (for ngrok routing)
  const normalizedPath = pathname.startsWith('/mxtk') ? pathname.slice(5) || '/' : pathname
  return (THEME_RULES.find(x => x.test(normalizedPath)) || THEME_RULES[0]).theme
}
