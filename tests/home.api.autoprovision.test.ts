...
describe('Home API autoprovision (contract expectations)', () => {
  it('seed presets include price-large and pools-table in overview; content widget in MXTK-Info-1', async () => {
    ...
    const info1 = doc.widgets.filter(w => w.sectionId === 'mxtk-info-1')
    expect(new Set(info1.map(w => w.type)).has('content-widget')).toBe(true)
  })
})
